...
{id:"e2025", year:"2025", title:"2025학년도 1학기 운영체제및실습 기말고사", meta:"2025.6.18 · 55분 · single CPU 대상", tag:"exam",
 note:"가장 최근 족보(직접 판독 복원). 손글씨 정답을 함께 반영함. 이미지 품질이 낮아 일부 보기 문장은 의미를 살려 복원([추론] 표기).", items:[
  ...
  {n:"2", pts:"10점", tags:["스케줄링","Round Robin","빈칸"],
   ...
   a:"**① 일정한(고정) 주기** — 타이머가 일정 간격(예: 0.001초)마다 동작\n**② 타이머 인터럽트(timer interrupt)** — 하드웨어 타이머가 주기적으로 발생\n**③ 인터럽트 핸들러(interrupt handler)** — 인터럽트 종류 식별·분기\n**④ 타이머 ISR(timer interrupt service routine)** — 타이머 전용 처리 루틴\n**⑤ 100** — 카운터가 100이면 time slice 경과\n\n**계산**: 목표값 = time slice ÷ 타이머 주기 = 0.1초 ÷ 0.001초 = **100**. (0.001초 인터럽트가 100번 누적 = 0.1초)",
   expl:"타이머 ISR이 호출될 때마다 카운터 +1, (time slice/타이머주기)에 도달하면 한 슬라이스 끝 → 스케줄러 호출. 함정: ⑤를 시간(0.1초)으로 쓰는 것 — 정수 100(횟수)이어야.",
   crit:"**⑤=100**(정수, 인터럽트 누적 횟수)과 의미(타이머 카운트로 time slice 경과 판단)가 필수. ②=타이머 인터럽트, ④=타이머 ISR 명시.\n**필수** — ⑤를 '0.1초'로 쓰면 오답(카운터는 시간이 아니라 횟수)."},
  ...
  {n:"3", pts:"6점", tags:["프로세스","상태도","서술"],
   ...
   a:"7상태 = 기본 5개(New, Ready, Running, Blocked, Terminate) + suspend 2개(Ready-suspend, Blocked-suspend).\nsuspend=swap-out(회수), activate=swap-in(할당), 생성=할당, 종료=회수.\n\n**메모리 할당/회수가 일어나는 6가지 전이**:\n1. **New → Ready** (생성, 할당)\n2. **Ready → Ready-suspend** (swap-out, 회수)\n3. **Blocked → Blocked-suspend** (swap-out, 회수)\n4. **Ready-suspend → Ready** (swap-in, 할당)\n5. **Blocked-suspend → Blocked** (swap-in, 할당)\n6. **Running/Ready/Blocked → Terminate** (종료, 회수)",
   expl:"메모리 변동은 생성(할당)·suspend(회수)·activate(할당)·종료(회수)에서 발생. 함정: Blocked-suspend↔Ready-suspend는 둘 다 디스크라 메모리 변동 없음 → 답 아님.",
   crit:"'~상태에서 ~상태로' 형식 + **실제 메모리 할당/회수가 일어나는 전이만** 선별.\n**필수** — New→Ready, Ready→Ready-suspend, Blocked→Blocked-suspend, Ready-suspend→Ready, Blocked-suspend→Blocked, ?→Terminate. (suspend↔suspend는 오답)"},
  ...
  {n:"5", pts:"10점", tags:["파일관리","inode","그림"],
   ...
   a:"**핵심 단서(블록 2048byte, 주소 4byte 가정)**: 한 인덱스 블록 슬롯 = 2048÷4 = **512개**.\n- inode의 direct는 데이터 블록을 **직접** 지정\n- single indirect: 인덱스 블록(512슬롯) → 데이터 블록\n- double indirect: 인덱스 → 인덱스 → 데이터 블록(예: 550)\n**풀이**: 각 화살표가 가리키는 블록 번호를 위→아래로 추적해 10칸을 채움. (정답 그림은 afig 참조)",
   expl:"inode 구조: direct(직접) / single·double indirect(인덱스 1·2단계). 한 인덱스 블록=블록크기/주소크기=2048/4=512. 함정: indirect는 데이터 전에 인덱스 블록을 먼저 읽어야 함(디스크 접근↑).",
   crit:"인덱스 블록 슬롯 = **2048/4 = 512** + 트리의 각 화살표가 가리키는 블록 번호 정확히 추적. direct/single/double indirect 구분.\n**필수** — 512 슬롯, indirect 단계 추적."},
...
{id:"e2023", year:"2023", title:"2023학년도 운영체제및실습 기말고사", meta:"2023.6.14 · 45분", tag:"exam", items:[
  ...
  {n:"2", pts:"10점", tags:["파일관리","Grouping","그림"],
   ...
   a:"**Grouping 구조**: 그룹 인덱스 블록 1개에 빈 블록 주소 여러 개 + 마지막 칸에 **다음 그룹 인덱스 블록 포인터**. super block은 현재 그룹 인덱스를 보관.\n\n**할당 절차(끝쪽부터)**:\n1. super block 리스트의 빈 블록 주소(예: 112)를 먼저 내줌\n2. 마지막에 남은 게 **다음 그룹 인덱스 포인터(예: 107)**이면, 107을 할당하기 전에 **107 안의 다음 그룹 리스트(210 등)를 super block으로 끌어올림**\n\n**할당 후 결과(핵심)**: super block 끝 **112 사라짐**, 데이터 블록 **107 사라짐**, **210이 super block의 새 그룹 인덱스로 승격**.",
   expl:"grouping=첫 블록(그룹 인덱스)에 빈 주소들+마지막은 다음 그룹 포인터. 함정: 그룹 인덱스 블록(107)을 데이터로 내주기 전 그 안의 '다음 그룹' 정보를 super block으로 복사 안 하면 나머지 리스트를 잃음.",
   crit:"**112 제거 + 107 제거 + 210이 super block 다음(새 그룹 인덱스)으로 승격** 세 가지 필수. 그룹 인덱스 블록 할당 시 다음 그룹 정보를 super block으로 올리는 메커니즘.\n**필수** — 112·107 할당, 210 승격."},
  ...
  {n:"3", pts:"30점 (각 2점)", tags:["프로세스","빈칸","page fault"],
   ...
   a:"page fault 처리 흐름 빈칸:\n**① present(valid) bit** — 페이지 메모리 적재 여부(0=부재)\n**② trap**(=page fault, 소프트웨어 인터럽트)\n**③ kernel(supervisor) 모드**\n**④ interrupt handler**\n**⑤ IVT/IDT**(interrupt vector/descriptor table)\n**⑥ device driver**\n**⑦ 파일(file)** — 장치를 파일처럼 취급\n**⑧ block(wait, sleep)**\n**⑨ process scheduler**(short-term)\n**⑩ running**\n**⑪ interrupt**(I/O 완료)\n**⑫ 2**(프로세스2)\n**⑬ ready**\n**⑭ process scheduler**\n**⑮ ③**(=process scheduler 선택지)",
   crit:"각 2점. ①present/valid ②trap ③kernel ④interrupt handler ⑤IVT/IDT ⑥device driver ⑦파일 ⑧block ⑨process scheduler ⑩running ⑪interrupt ⑫2 ⑬ready ⑭process scheduler ⑮③.\n**필수** — ①valid/present, ②trap, ⑧block, ⑬ready 구분.",
   expl:"page fault 전체 흐름 암기형: present bit 확인→trap→커널 모드→interrupt handler→IVT/IDT로 ISR→device driver에 I/O(장치=파일)→P1 block→scheduler가 P2 선택→running·context switch→완료 interrupt→P1 ready 복귀→scheduler. 함정: ①valid/present, ⑧block(running 아님), ⑬ready."},
